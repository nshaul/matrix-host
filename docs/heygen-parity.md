...
| Own voice, free tier | Yes: system voice free and offline; Kokoro-82M neural TTS free, local, Apache-2.0| Voice cloning exists but is a paid platform feature |
| ElevenLabs support | Yes, two ways: your key in the voice panel or any server proxy handing audio to `speak()` (works today) | Yes, ElevenLabs integration is supported on their platform |
| Spoken command control ("wave", "look left") | Yes, VoiceCommands via the Web Speech API | No spoken operator control; interaction is chat/LLM-driven conversation |
